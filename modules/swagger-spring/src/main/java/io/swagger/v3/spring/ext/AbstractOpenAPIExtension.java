package io.swagger.v3.spring.ext;
...
import io.swagger.v3.spring.ResolvedParameter;
...
public abstract class AbstractOpenAPIExtension implements OpenAPIExtension {
    ...
    @Override
    public ResolvedParameter extractParameters(List<Annotation> annotations, Type type, Set<Type> typesToSkip, Components components,
    ...
    if (chain.hasNext()) {
            return chain.next().extractParameters(annotations, type, typesToSkip, components, classAnnotation, methodAnnotation, includeRequestBody, jsonViewAnnotation, chain);
        } else {
